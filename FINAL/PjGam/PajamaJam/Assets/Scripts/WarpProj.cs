...
public class WarpProj : MonoBehaviour {
	...
	void OnCollisionEnter(Collision hunter){
		...
		//print ("asd");
		if (!hunter.gameObject.CompareTag ("Border") && !hunter.gameObject.CompareTag ("Star")
		    && !hunter.gameObject.CompareTag ("Portal")) {
			...
			if (hunter.gameObject.tag == "Hunter" || hunter.gameObject.tag == "Runner") {
				AudControl aud = hunter.gameObject.GetComponent<AudControl>();
				aud.Play(3,false);
